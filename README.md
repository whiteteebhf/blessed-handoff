...
| Kimi Code CLI · macOS · headless one-shot pickup (`kimi -p`) | **Tested** (2026-07-22, v1.1.0): both skills install unmodified at `$KIMI_CODE_HOME/skills/`; doc write, cold-reader gate, spawn, and ACK pickup verified end-to-end |
| Kimi Code CLI · macOS · WezTerm interactive successor | **Tested** (2026-07-24): `PROMPT_STYLE=skill-command` (spawn bare → `/handoff-resume` as first TUI input). Note `kimi` has **no positional prompt arg**, and `-p` is one-shot/non-interactive, so interactive handoffs must not use it |
