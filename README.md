...
| Kimi Code CLI · macOS · WezTerm auto-spawn | **Tested** — headless one-shot pickup via `kimi -p` (2026-07-22, v1.1.0): both skills install unmodified at `$KIMI_CODE_HOME/skills/`; doc write, cold-reader gate, spawn, and ACK pickup verified end-to-end. Interactive successor delivery via `PROMPT_STYLE=skill-command` (spawn bare → `/handoff-resume` as first TUI input) verified live 2026-07-24 — note `kimi` has **no positional prompt arg**, and `-p` is one-shot/non-interactive, so interactive handoffs must not use it |
